#!/usr/bin/env bash
# Builds Kabigon.app and a DMG. Signs with a Developer ID and notarizes when
...
if [ -n "${SIGN_IDENTITY:-}" ]; then
    ...
    codesign --force --options runtime --timestamp --sign "$SIGN_IDENTITY" "$APP/Contents/MacOS/kabigon"
    codesign --force --options runtime --timestamp --sign "$SIGN_IDENTITY" "$APP"
    codesign --verify --strict "$APP"
fi

DMG="build/Kabigon-$VERSION.dmg"
STAGE="build/dmg"
rm -f "$DMG"; rm -rf "$STAGE"; mkdir -p "$STAGE"
cp -R "$APP" "$STAGE/Kabigon.app"
ln -s /Applications "$STAGE/Applications"
hdiutil create -volname "Kabigon" -srcfolder "$STAGE" -ov -format UDZO "$DMG" >/dev/null
