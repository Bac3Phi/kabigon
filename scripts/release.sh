...
#   xcrun notarytool store-credentials kabigon \
...
done < <(find "$APP" -name "*.bundle" -print0 2>/dev/null)
...
codesign --force --options runtime --timestamp --sign "$IDENTITY" "$APP/Contents/MacOS/kabigon"
codesign --force --options runtime --timestamp --sign "$IDENTITY" "$APP"
codesign --verify --deep --strict --verbose=2 "$APP"

DMG="$ROOT/build/Kabigon-$VERSION.dmg"
STAGE="$ROOT/build/dmg"

make_dmg() {
    rm -f "$DMG"; rm -rf "$STAGE"; mkdir -p "$STAGE"
    # ditto preserves the stapled notarization ticket inside the app bundle.
    ditto "$APP" "$STAGE/Kabigon.app"
    ...
    create-dmg \
        ...
        --window-size 660 420 \
        --icon-size 120 \
        --icon "Kabigon.app" 180 206 \
        --app-drop-link 480 206 \
        --hide-extension "Kabigon.app" \
        --no-internet-enable \
        "$DMG" "$STAGE" >/dev/null
}
...
if [ -n "${SKIP_NOTARIZE:-}" ]; then
    ...
    ZIP="$ROOT/build/Kabigon-$VERSION-app.zip"
    rm -f "$ZIP"
...
cat <<EOF
        <item>
            ...
            <enclosure url="https://github.com/Bac3Phi/kabigon/releases/download/v$VERSION/Kabigon-$VERSION.dmg"
