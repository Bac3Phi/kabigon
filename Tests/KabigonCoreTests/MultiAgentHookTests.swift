...
final class MultiAgentHookTests: XCTestCase {
    private let cmd = "\"/Applications/Kabigon.app/Contents/MacOS/kabigon\" hook --agent cursor"
    ...
    func testCursorInstallShape() {
        ...
        XCTAssertTrue((stop?.first?["command"] as? String ?? "").contains("kabigon"))
    }
    ...
    func testWindsurfInstallShape() {
        ...
        let cmd = "\"/x/kabigon\" hook --agent windsurf"
    ...
    func testOpencodeBinaryPathExtraction() {
        XCTAssertEqual(
            HookInstaller.binaryPath(fromCommand: "\"/Applications/Kabigon.app/Contents/MacOS/kabigon\" hook --agent opencode"),
            "/Applications/Kabigon.app/Contents/MacOS/kabigon")
    }

    func testOpencodePluginContent() {
        let js = HookInstaller.opencodePlugin(binary: "/x/kabigon")
    ...
    func testCodexPayloadDecode() {
        let json = #"{"session_id":"s1","cwd":"/proj","hook_event_name":"PreToolUse","tool_name":"Bash"}"#
        let ev = HookPayload.event(forAgent: .codex, stdin: Data(json.utf8), now: Date())
        XCTAssertEqual(ev?.sessionId, "s1")
        XCTAssertEqual(ev?.eventName, "PreToolUse")
        XCTAssertEqual(ev?.project, "/proj")
        XCTAssertEqual(ev?.message, "Using Bash")
        XCTAssertEqual(ev?.agentKind, .codex)
    }

    func testCodexPayloadDecodeAlternateFieldNames() {
        let json = #"{"thread_id":"t1","project":"/proj","event_name":"PermissionRequest","message":"Needs approval"}"#
        let ev = HookPayload.event(forAgent: .codex, stdin: Data(json.utf8), now: Date())
        XCTAssertEqual(ev?.sessionId, "t1")
        XCTAssertEqual(ev?.eventName, "PermissionRequest")
        XCTAssertEqual(ev?.project, "/proj")
        XCTAssertEqual(ev?.message, "Needs approval")
        XCTAssertEqual(StateMapper.state(for: .codex, eventName: ev!.eventName), .waiting)
    }

    func testAugmentPayloadDecode() {
        let json = #"{"conversation_id":"c1","workspace_roots":["/proj"],"hook_event_name":"PreToolUse","tool_name":"launch-process"}"#
        let ev = HookPayload.event(forAgent: .augment, stdin: Data(json.utf8), now: Date())
        XCTAssertEqual(ev?.sessionId, "c1")
        XCTAssertEqual(ev?.eventName, "PreToolUse")
        XCTAssertEqual(ev?.project, "/proj")
        XCTAssertEqual(ev?.message, "Using launch-process")
        XCTAssertEqual(ev?.agentKind, .augment)
    }
    ...
    func testAugmentStateMappingAndEvents() {
        let events = AgentHooks.spec(for: .augment)!.events
        XCTAssertTrue(events.contains("PostToolUse"))
        XCTAssertFalse(events.contains("AfterTool"))
        XCTAssertEqual(StateMapper.state(for: .augment, eventName: "SessionStart"), .registered)
        XCTAssertEqual(StateMapper.state(for: .augment, eventName: "PreToolUse"), .working)
        XCTAssertEqual(StateMapper.state(for: .augment, eventName: "PostToolUse"), .working)
        XCTAssertEqual(StateMapper.state(for: .augment, eventName: "Notification"), .waiting)
        XCTAssertEqual(StateMapper.state(for: .augment, eventName: "Stop"), .done)
    }

    func testHermesStateMapping() {
        XCTAssertEqual(StateMapper.state(for: .hermes, eventName: "SessionStart"), .registered)
        XCTAssertEqual(StateMapper.state(for: .hermes, eventName: "UserPromptSubmit"), .working)
        XCTAssertEqual(StateMapper.state(for: .hermes, eventName: "PreToolUse"), .working)
        XCTAssertEqual(StateMapper.state(for: .hermes, eventName: "Notification"), .waiting)
        XCTAssertEqual(StateMapper.state(for: .hermes, eventName: "Stop"), .done)
        XCTAssertEqual(StateMapper.state(for: .hermes, eventName: "SubagentStop"), .done)
    }
    ...
    func testIsSessionEnd() {
        ...
        XCTAssertTrue(StateMapper.isSessionEnd(for: .hermes, eventName: "SessionEnd"))
    ...
    func testDiskRoundTripAllStyles() throws {
        let tmp = NSTemporaryDirectory() + "kabigon-test-\(UUID().uuidString)/"
        defer { try? FileManager.default.removeItem(atPath: tmp) }
        let cases: [(AgentKind, String)] = [(.cursor, "cursor.json"), (.windsurf, "windsurf.json"), (.opencode, "plugin/kabigon.js"), (.augment, "augment.json"), (.hermes, "hermes.json")]
        for (kind, file) in cases {
            let spec = AgentHooks.spec(for: kind)!
            let path = tmp + file
            let command = "\"/Applications/Kabigon.app/Contents/MacOS/kabigon\" hook --agent \(kind.rawValue)"
