...
final class EventSenderTests: XCTestCase {
    func testSenderDeliversOverSocket() throws {
        let path = "/tmp/kabigon-\(UUID().uuidString).sock"
    ...
    func testSenderFallsBackToQueueWhenNoServer() throws {
        let socketPath = "/tmp/kabigon-missing-\(UUID().uuidString).sock"
        let queueDir = NSTemporaryDirectory() + "kabigon-q-\(UUID().uuidString)"
