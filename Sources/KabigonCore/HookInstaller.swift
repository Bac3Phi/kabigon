...
/// Installs/removes Kabigon's hook entries in an agent's config. Claude Code,
...
public enum HookInstaller {
    ...
    static func isOurs(_ command: String) -> Bool {
        (command.contains("kabigon") || command.contains("agentpet")) && command.contains("hook")
    }
    ...
    public static func isInstalled(in settings: [String: Any], events: [String] = events) -> Bool {
        ...
        for event in events {
            guard let groups = hooks[event] as? [[String: Any]],
                  groups.contains(where: groupIsOurs)
            else { return false }
        }
        return true
    }
    ...
    static func isInstalledFlat(in settings: [String: Any], events: [String]) -> Bool {
        ...
        guard let hooks = settings["hooks"] as? [String: Any] else { return false }
        for event in events {
            guard let items = hooks[event] as? [[String: Any]],
                  items.contains(where: flatItemIsOurs)
            else { return false }
        }
        return true
    }
    ...
    /// Extracts the Kabigon binary path from a hook command like
    /// `"/path/to/kabigon" hook --agent opencode` (the first quoted token).
    ...
    static func opencodePlugin(binary: String) -> String {
        """
        // Kabigon integration (auto-generated, safe to delete to uninstall).
        // Reports opencode session lifecycle to Kabigon's menu bar app.
        const KABIGON_BIN = \(jsString(binary))
        export const Kabigon = async ({ directory }) => {
          const sid = "opencode:" + (directory || "default")
          const send = (state) => {
            try {
              Bun.spawn([KABIGON_BIN, "hook", "--agent", "opencode",
