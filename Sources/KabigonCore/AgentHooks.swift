...
public enum AgentHooks {
    public static func spec(for kind: AgentKind) -> AgentHookSpec? {
        ...
        case .augment:
            // Augment uses the same nested hook shape as Claude Code.
            return AgentHookSpec(
                kind: .augment, style: .claudeNested,
                events: ["SessionStart", "PreToolUse", "PostToolUse", "Notification", "Stop", "SessionEnd"],
                settingsPath: home + "/.augment/settings.json")
        case .hermes:
            // Hermes uses the same nested hook shape as Claude Code.
            return AgentHookSpec(
                kind: .hermes, style: .claudeNested,
                events: ["SessionStart", "UserPromptSubmit", "PreToolUse", "Notification", "Stop", "SubagentStop", "SessionEnd"],
                settingsPath: home + "/.hermes/hooks.json")
        case .cli, .unknown:
            return nil
        }
    }
