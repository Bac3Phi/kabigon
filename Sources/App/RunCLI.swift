...
/// `kabigon run [--session id] [--project path] [--agent kind] -- <command...>`
///
    ...
    static func run(arguments: [String]) -> Never {
        ...
        guard !parsed.command.isEmpty else {
            FileHandle.standardError.write(Data(
                "usage: kabigon run [--session id] [--project path] [--agent kind] -- <command...>\n".utf8))
            exit(2)
        }
        ...
        func emit(_ state: String) {
            ...
            EventSender.send(event, socketPath: KabigonPaths.socketPath, queueDir: KabigonPaths.queueDir)
        }
        ...
        do {
            ...
            heartbeat.cancel()
            emit("done")
            FileHandle.standardError.write(Data("kabigon run: failed to launch \(parsed.command.first ?? "")\n".utf8))
            exit(126)
        }
